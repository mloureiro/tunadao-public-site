// Mashup Template Exports
// Blue Note heritage + Advanced refinement + Experimental boldness
// Inspirations: Blue Note Records, Advanced, Experimental

// Shell
export { default as Layout } from './layouts/Layout.astro';
export { default as Hero } from './components/Hero.astro';
export { default as Header } from './components/Header.astro';
export { default as Footer } from './components/Footer.astro';
// Content (temporary — will be replaced in Phase 5b)
export { default as EditionCard } from '@components/citadao/EditionCard.astro';
export { default as FestivalCard } from '@components/palmares/FestivalCard.astro';
export { default as Award } from '@components/Award.astro';
export { default as Section } from '@components/Section.astro';
export { default as YearNav } from '@components/VerticalTimeline.astro';
